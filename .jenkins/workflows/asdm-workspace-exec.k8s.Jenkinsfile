...
//
// 默认联调：workspace id 12；若 ASDM 网关跑在宿主机上，K8s pod 内应使用 http://host.docker.internal:8880 访问。
// asdm 进程同时读 ~/.asdm/config 与环境变量 BASE_URL、ARTIFACT_BASE_URL（与 asdm-cli 一致）。
//
// 前置：
// 1) Agent 镜像需预装 codebuddy、codebuddy-log-parser、asdm（见 Dockerfile.agent 改造方案）
...
pipeline {
  ...
  parameters {
    ...
    string(
      ...
      description: 'ASDM 平台根地址；会写入 asdm config 并 export BASE_URL 供 CLI 使用（k3d 内访问宿主机用 host.docker.internal）'
    )
    string(
    ...
    stage('Verify agent toolchain') {
      steps {
        sh '''bash -s <<'ASDM_SCRIPT'
          set -euo pipefail
          set -x
          echo "=== Workspace exec / agent 自检 ==="
          node --version
          npm --version
          git --version
          python3 --version
          codebuddy --version
          command -v asdm >/dev/null 2>&1 || { echo "缺少 asdm 命令：请按 ASDM_WORKSPACE_EXECUTION_PLAN 改造 agent 镜像" >&2; exit 1; }
          asdm --version
ASDM_SCRIPT
        '''
      }
...
ASDM_SCRIPT
    ...
    stage('Configure ASDM and install workspace') {
      steps {
        script {
          def bu = params.ASDM_BASE_URL?.trim() ?: 'http://host.docker.internal:8880'
...
ASDM_SCRIPT
          ...
          // 优先级：构建参数 > 已存在环境变量（用户已在 Jenkins 全局/Pod 里配置） > Jenkins 凭据
...
{
  "pipeline": "asdm-workspace-exec.k8s",
